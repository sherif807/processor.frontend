...
import CommentsComponent from './CommentsComponent'; // Import the comments component

export default function PendingItemsPage({ page, setTotalItems }) {
  const [pictures, setPictures] = useState([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const fetchPendingPictures = async () => {
      const apiUrl = process.env.NEXT_PUBLIC_API_URL;
      setLoading(true);
      try {
        const response = await fetch(`${apiUrl}/api/pictures?&order[uploadedAt]=desc&listingStatus=2&page=${page}`);
  ...
  const bringBackPicture = async (id) => {
    ...
    try {
      const response = await fetch(`${apiUrl}/api/pictures/${id}`, {
        ...
        body: JSON.stringify({ listingStatus: 0 }),
      });
      if (!response.ok) {
        throw new Error('Network response was not ok');
      }
            ...
            {picture.catalogItems.length > 0 ? (
              picture.catalogItems.map((item, index) => (
                <div key={item['@id']} className="border-t border-gray-200 mt-4 relative">
                  ...
                  {/* Inject the CommentsComponent under each item, making it span full width */}
